...
<main className="px-2 sm:px-16 md:px-28 lg:px-56 mt-16 md:mt-20">
  ...
  {/* Main Blog */}
<div className="relative flex-1 shadow-lg rounded-md bg-gray-50 dark:bg-darkBg p-4 ring ring-gray-100 dark:ring-gray-700">
    ...
    <div className="flex gap-1">
      {Array.isArray(data.category) ? (
        data.category.map((cat, index) => (
          <p key={index} className="bg-gray-300 dark:bg-gray-600 text-darkBg dark:text-gray-50 px-1 rounded">
            {cat}
          </p>
        ))
      ) : (
        <p className="bg-gray-300 dark:bg-gray-600 text-darkBg dark:text-gray-50 px-4 py-1 rounded">{data.category}</p>
      )}
    </div>
  </div>
  ...
  {/* Title, Description, Image, etc. */}
      ...
      {/* Title */}
      <h1 className="text-cyan-500 text-3xl md:text-4xl lg:text-5xl font-bold py-2">{data.title}</h1>

      {/* Description + Image */}
      <p className="text-md text-gray-600 dark:text-gray-100 mb-4">{data.description}</p>
    ...
    <hr/>
      ...
      {relatedBlogs.map((blog, index) => (
        <section key={index} className="shadow-md rounded-md bg-gray-50 dark:bg-darkBg ring ring-gray-100 dark:ring-gray-700 overflow-hidden">
          ...
          <div className="p-3 text-darkBg dark:text-gray-50">
            ...
            <Link href={blog.link} className="text-cyan-500 hover:underline text-sm">
              Read Article
            </Link>
          </div>
  ...
  {/* Comment Section */}
  <section className="bg-gray-100 dark:bg-darkBg p-5 border-t border-gray-200 dark:border-gray-600 mt-8 rounded-md">
